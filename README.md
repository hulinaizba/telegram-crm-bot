...
| Команда | Назначение |
|---|---|
| `/new` | Пошаговое добавление клиента (диалог, включая заметку и напоминание) |
...
| `/clients` | Список всех клиентов — кнопками открывается карточка и редактирование |
...
### Интерактивная карточка клиента

`/clients` показывает список с кнопками — тап по клиенту открывает карточку
(та же сводка, что и `/complete`) с меню редактирования: изменить имя, опыт,
терминал, депозит, формат торговли, добавить заметку или сменить этап
(отдельным меню с 7 кнопками этапов). Значение вводится обычным текстом в
ответ на запрос бота — те же самые `/edit`, `/note`, `/setstage` под
капотом, просто без необходимости печатать команду с параметрами вручную.

### Заметка и напоминание при создании клиента

После шага 6/6 (способ торговли) диалог `/new` предлагает ещё два шага:
свободную заметку («-» — пропустить) и разовое напоминание — через сколько
дней (кнопки 1–7 или своё число) прислать в `/today` сигнал 🔔 по этому
клиенту в 08:00. То же самое для уже существующих клиентов — команда
`/remind @username N` (или `/remind @username -`, чтобы снять). Дата
напоминания хранится в таблице и переживает перезапуск бота; после показа
в дайджесте снимается автоматически — это разовое, а не повторяющееся
напоминание.
...
tests/         — pytest, 121 тестов на фейковой таблице и фейковых Update
...
- `pytest` — 121 unit-тестов без обращения к реальным Telegram/Google API
...
4. Создайте лист (по умолчанию `Клиенты`) с заголовками первой строки:
   ...
   заметки, дата_старта, последний_контакт, статус, chat_id, дата_напоминания`.
